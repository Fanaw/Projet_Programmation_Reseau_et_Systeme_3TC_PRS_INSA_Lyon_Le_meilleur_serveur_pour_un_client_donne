    ...
    if( (desc = socket(PF_INET,SOCK_DGRAM,0)) > 0){
        ...
        if( (reuseok = setsockopt(desc,SOL_SOCKET,SO_REUSEADDR,&reuse,sizeof(reuse))) >= 0){
            ...
            while(1){
                ...
                printf("En attente du SYN pid : %d\n",getpid());
                ...
                if(strcmp(buffer,"SYN") == 0){ //Si on a bien reçu SYN on continue 
                    ...
                    strcat(syn_ack, port);
                    ssize_t sendsyn = sendto(desc,syn_ack,sizeof(syn_ack),0,(struct sockaddr*)&my_addr.sin_addr, cli_len);
                    ...
                    if(strcmp(buffer,"ACK") == 0){
                        ...
                        if(pid == 0){
                            printf("je suis dans le fils port : %d\n",numport);
                            int desc2;
                            if( (desc2 = socket(PF_INET,SOCK_DGRAM,0)) > 0){ //on ouvre une deuxième socket privée
                                ...
                                if( (reuseok2 = setsockopt(desc2,SOL_SOCKET,SO_REUSEADDR,&reuse,sizeof(reuse))) >= 0){
                                    ...
                                    //Bind socket2
                                    if(bind(desc2,(struct sockaddr*)&my_addr2,sizeof(my_addr2))==0){
                                    printf("Bind2 au port %d\n",numport);
                                    }                                
...
int slowStart(char* file,int desc,  struct sockaddr_in my_addr, int cli_len){
    ...
    while(nbelemrecu==nbElements){ //tant que ce n'est pas la fin du fichier
        ...
        struct timeval temps_avant, temps_apres, diff; //on lance un timer
        ...
        }while((((temps_apres.tv_sec - temps_avant.tv_sec) * 1000000 + temps_apres.tv_usec) - temps_avant.tv_usec)<2000 && max<seq);
